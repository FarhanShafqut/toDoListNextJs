...
const deleteTodos = (title)=>{
  let newTodos = todos.filter(item =>{
    // If item.title is not equal to the provided title, the comparison item.title !== title evaluates to true, and that item is included in the newTodos array.
  ...
  <div className="container px-5 py-24 mx-auto">
    <div className="flex flex-col text-center w-full mb-20">
      ...
      <p className="lg:w-2/3 mx-auto leading-relaxed text-base">Yours Todos will show up here </p>
    </div>
    <div className="flex flex-wrap -m-4">
      {todos.map(item=>{
       return <div className="p-4 lg:w-1/4 md:w-1/2">
        <div className="h-full flex flex-col items-center text-center">
          <img alt="team" className="flex-shrink-0 rounded-lg w-full h-56 object-cover object-center mb-4" src={`https://picsum.photos/700/900
?a=${item.des}`}/>
          <div className="w-full">
            <h2 className="title-font font-medium text-lg text-gray-900">{item.title}</h2>
            <p className="mb-4">{item.des}</p>
            <span className="inline-flex">
              <a className="text-gray-500 cursor-pointer " onClick={()=>{deleteTodos(item.title)}}>
